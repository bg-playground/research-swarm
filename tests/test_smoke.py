...
def test_gatherer_node_with_mock_scrape():
    ...
    assert result.goto == "supervisor"
    assert result.update["sources"][0].markdown is not None
...
def test_synthesizer_node_fallback_without_llm():
    ...
    facts = [
        ExtractedFact(claim="Example claim", value="value", source_urls=["https://example.com"], confidence=0.7)
    ]
...
def test_extractor_skips_already_extracted_urls():
    ...
    state = _minimal_state(sources=[src], extracted_urls=["https://docs.example.com/api"])
...
def test_coverage_route_prefers_gatherer_when_unscored():
    from src.agents.supervisor import _coverage_route
    sources = [
        Source(url="https://a.example", markdown="# done"),
        Source(url="https://b.example"),
        Source(url="https://c.example"),
    ]
    state = _minimal_state(
        sources=sources,
        extracted_urls=["https://a.example"],
        extracted_facts=[],
        iteration=2,
        max_iterations=8,
    )
    assert _coverage_route(state) == "gatherer"


def test_coverage_route_prefers_extractor_when_pending():
    from src.agents.supervisor import _coverage_route
    sources = [Source(url="https://a.example", markdown="# new content")]
    state = _minimal_state(sources=sources, extracted_urls=[], iteration=2, max_iterations=8)
    assert _coverage_route(state) == "extractor"
...
def test_build_search_queries_docs_bias():
    ...
    qs = build_search_queries(
        "From docs.firecrawl.dev, summarize search scrape crawl API", max_queries=3
    )
